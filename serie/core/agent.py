...
from serie.utils.io import load_jsonl, save_jsonl, import_config
...
class Agent:
    def __init__(self, model: str):
        configs = import_config(get_class_config_file_path(self.__class__))
        self.model = model
    ...
    def __call__(
            ...
            mode: str | TaskMode = TaskMode.SINGLE,
            **kwargs):
        if isinstance(mode, str):
            mode = TaskMode(mode)
        if mode == TaskMode.SINGLE:
            assert isinstance(message, str)
            return self.complete_single(message, **kwargs)
        elif mode == TaskMode.BATCH:
            assert isinstance(message, list)
            return self.complete_batch(message, **kwargs)
        elif mode == TaskMode.CONCURRENT:
            assert isinstance(message, list)
            return self.complete_concurrent(message, **kwargs)
        else:
    ...
    def complete_batch(self, messages: list[str], **kwargs) -> list[str]:
